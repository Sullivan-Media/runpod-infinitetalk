...
def select_output_video(videos):
    ...
    멀티윈도우 작업은 첫 윈도우 프리뷰(_00001.mp4)와 전체 muxed 영상
    (_00002-audio.mp4)을 함께 생성한다. '첫 노드'를 고르면 짧은 프리뷰가
    반환되므로, muxed(-audio) 파일을 우선하고 없으면 최대 크기(=최장) 파일을 고른다.
    """
    all_videos = [
        p
    ...
    if not all_videos:
        return None
    # ponytail: 크기를 길이의 프록시로 사용. 정확한 길이가 필요하면 ffprobe로 교체.
    audio_videos = [p for p in all_videos if "-audio" in os.path.basename(p)]
    candidates = audio_videos or all_videos
...
def handler(job):
    ...
    if output_video_path:
        logger.info(
            f"선택된 출력 비디오: {output_video_path} "
            f"(크기: {os.path.getsize(output_video_path)} bytes)"
        )
