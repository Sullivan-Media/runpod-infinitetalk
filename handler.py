...
def select_output_video(videos):
    """Pick the final audio-muxed result (VHS '-audio' suffix, node 131 in i2v.json);
    fall back to the first non-empty output node."""
    fallback = None
    for paths in videos.values():
        for p in paths:
            if "-audio" in os.path.basename(p):
                return p
            if fallback is None:
                fallback = p
    return fallback
...
def handler(job):
    ...
    # 동적 Force Offload 설정
    # ------------------------------------------------------------------
    # 1. 입력에서 force_offload 읽기 (기본값 False: 32GB GPU에 모델 상주, 윈도우마다 재로드 방지)
    ...
    # 최종 오디오 합성 결과(노드 131, '-audio' 접미사)를 우선 선택. 없으면 첫 번째 비디오로 폴백.
    output_video_path = select_output_video(videos)
    if output_video_path:
        logger.info(f"출력 비디오 선택됨: {output_video_path}")
