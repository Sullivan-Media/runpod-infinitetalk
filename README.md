...
| `force_offload` | `boolean` | No | `false` | Whether to offload model components to CPU during inference. Default `false` keeps weights resident for faster processing on high-VRAM GPUs (the workflow keeps the full transformer on-GPU). Set to `true` to prevent OOM on smaller GPUs. |
